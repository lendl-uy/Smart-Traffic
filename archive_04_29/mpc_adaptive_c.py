...
def do_mpc(x_curr=np.array([0,0,0,0,0]), step=0):
    ...
    if sum(x_curr) <= 0.0:
        ...
        u_2mult = 1+(d_total_katip/d_total)
        u_3mult = 1+(d_sum_aurora[0]/d_total)
        u_4mult = 1+(d_sum_aurora[1]/d_total)
        u_41multiplier = min(u_2mult, u_3mult, u_4mult, 1)
    else:
        ...
        u_2mult = 1+((x_curr[0]+x_curr[1])/x_total)
        u_3mult = 1+(x_curr[2]/x_total)
        u_4mult = 1+(x_curr[3]/x_total)
        u_41multiplier = 1+(x_curr[4]/x_total)
