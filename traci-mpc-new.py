...
def main():
    ...
    for step in range(sim_duration+1):
        ...
        qt_katip_south, qt_katip_north, qt_aurora_west, qt_aurora_east,cumulwait = perf.get_avg_wait()
        print("Cumulative Average Queue wait", cumulwait)
        ...
        if step%1800==0:
            list_of_ql_15_window.append(fifteeenqueuelength) #LIST OF QUEUE LENGTH PER 15 MINS
            fifteenlen=0
        ...
        if step%1800==0 and step>0:
            qtfifteen.append(sum(tempawl.values())/len(tempawl))
            for i in range(len(arrivedid)):
                tempawl.pop(arrivedid[i],None)
        
        print("qtfifteen",qtfifteen)#QUEUE TIME LIST PER 15 MINS
