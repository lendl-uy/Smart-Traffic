...
def do_mpc(x_curr=np.array([0,0,0,0]), step=0):
    ...
    #C = 300
    C = m.addVar(vtype=GRB.INTEGER, lb=100.0, ub = 300.0, name="C")
    ...
    additive = 20 # Dummy additive of 20 s
    for i in range(len(u_res)):
        u_res[i] = int(u_res[i]+0.5) # Convert to integer
        print(f"u_res = {u_res[i]}")
        if u_res[i] < 20:
            if u_res[i] < 0:
               additive = 0-(u_res[i])+20
            u_res[i] += additive
            print("Model was relaxed")
            continue
        #if relaxed_and_zero:
            #u_res[i] += 15 # Apply the same additive to nonzero green times
